...
> = {
  ...
  kolor: {
    ...
    steps: 20,
    guidance: 7.5,
    desc: "Kwai Kolors via SiliconFlow",
  ...
  "google-nano-banana-2": {
    ...
    steps: 1,
    guidance: 1,
    desc: "Google image gen via Gemini 2.5 Flash (AI Gateway)",
    provider: "gemini",
  },
  "gemini-2.5-flash-image": {
    ...
    steps: 1,
    guidance: 1,
    desc: "Google Gemini image generation",
    provider: "gemini",
  },
  "gemini-3.1-flash-image-preview": {
    ...
    steps: 1,
    guidance: 1,
    desc: "Gemini preview image model",
    provider: "gemini",
  },
};
...
> = {
  "production-watercolor": {
    name: "Production Watercolor",
    emoji: "🖌️",
    prompt:
      "a high-quality watercolor illustration of a single {word} for a children's vocabulary flashcard, soft warm pastel tones, gentle ink outlines, centered composition, plain cream paper background, child-friendly, no text, no labels",
    negPrompt:
      "photo, photograph, realistic, 3D render, scary, violent, text, letters, labels, watermark, busy background, multiple objects, human, hands, face, anthropomorphic",
  },
  realistic: {
  ...
  const buildUrl = useCallback(
    (word: Word, modelKey: string, prompt: string, negPrompt: string, g: number, s: number) => {
      ...
      if (selected.provider) {
        params.set("provider", selected.provider);
      }
          ...
          {toolTab === "images" ? (
            <>
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Compare Cloudflare, Gemini, and SiliconFlow image models with shared prompt and parameter controls.
                </p>
              </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button onClick={handleCompareModels} disabled={isRunning || !singleWord.trim()}>
                    <Layers className="h-4 w-4 mr-1" /> Compare {modelCount} Models
                  </Button>
                  <Button
                    variant="secondary"
                  ...
                  <Button
                    ...
                    disabled={isRunning || !singleWord.trim()}
                  >
                    <BarChart3 className="h-4 w-4 mr-1" /> Full Matrix ({modelCount}×{promptCount} = {modelCount * promptCount})
                  </Button>
                </div>
              </CardContent>
            </Card>
